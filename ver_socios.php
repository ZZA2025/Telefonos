...
$sql = "SELECT s.id, s.nombre, s.apellido, ts.tipo AS tipo_socio, ts.monto AS valor_tipo_socio, s.estado_cuenta,
               s.fecha_inicio, GROUP_CONCAT(DISTINCT ps.mes_pago ORDER BY ps.mes_pago ASC) AS pagos_realizados,
               GROUP_CONCAT(a.nombre SEPARATOR ', ') AS actividades, SUM(a.precio) + ts.monto AS cuota
        FROM socios s
...
function obtener_estado_suscripcion($fecha_inicio, $pagos_realizados, $estado_cuenta) {
    if ($estado_cuenta == 'Pago') {
        return 'Activa';
    }

    if (is_null($fecha_inicio) || empty($pagos_realizados)) {
        return 'Deuda';
    }
    ...
    $pagos_realizados = explode(',', $pagos_realizados);
    $ultimo_pago = end($pagos_realizados);

    if ($ultimo_pago) {
        $ultimo_pago = new DateTime($ultimo_pago);
...
function obtener_meses_adeudados($fecha_inicio, $pagos_realizados) {
    $fecha_actual = new DateTime();
    ...
    if (empty($meses_adeudados)) {
        return '<span style="color: green;">Al día</span>';
    } else {
        return '<span style="color: red;">' . implode(', ', $meses_adeudados) . '</span>';
    }
}
?>
                        ...
                        <?php if ($result->num_rows > 0): ?>
                            <?php while($row = $result->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($row['actividades'] ?? 'N/A'); ?></td>
                                    <td><?php echo number_format($row['cuota'] ?? 0, 2); ?></td>
                                    <td class="<?php echo $row['estado_cuenta'] == 'Pago' ? 'status-efectivo' : 'status-no-pago'; ?>">
                                        <?php echo htmlspecialchars($row['estado_cuenta']); ?>
                                    </td>
                                    <td><?php echo obtener_meses_adeudados($row['fecha_inicio'], $row['pagos_realizados']); ?></td>
                                    <td><a href="ver_estado_socio.php?id=<?php echo $row['id']; ?>" class="btn btn-info">Ver Estado</a></td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr>
